...
using Project_Redmil_MVC.Models.RequestModel.HealthBillRequestModel;
using Project_Redmil_MVC.Models.ResponseModel;
using Project_Redmil_MVC.Models.ResponseModel.ElectricityBillResponseModel;
using Project_Redmil_MVC.Models.ResponseModel.HealthBillResponseModel;
using System.Net.NetworkInformation;

namespace Project_Redmil_MVC.Controllers.BillPayments.HealthBillController
{
    public class HealthBillController : Controller
    {
        private readonly string Baseurl;
        private readonly IConfiguration _config;
        public HealthBillOperatorListResponseModel healthBillOperatorListResponseModel;
        public HealthBillController(IConfiguration config)
        {
            _config = config;
            Baseurl = HelperMethod.GetBaseURl(_config);
        }
        public IActionResult HealthBill()
        {
        ...
        public List<Operatornames> GetOperatorList()
        {
            ...
            HealthBillOperatorListRequestModel requestModel = new HealthBillOperatorListRequestModel();
            List<Operatornames> OpList = new List<Operatornames>();
            try
            {
                requestModel.Userid = "NA";
                requestModel.State = "";
                requestModel.Category = "Health Insurance";
                ...
                if (deserialize.Statuscode == "TXN" && deserialize != null)
                {
                    ...
                    var deserializeN = JsonConvert.DeserializeObject<HealthBillOperatorListResponseModel>(datadeserialize.ToString());
        ...
        public JsonResult FetchBill(string Number, string Input1, string Input2, string Operator, string ccf, string Amount, string Payment)
        {
            ...
            try
            {
                ...
                {
                    var datadeserialize = deserialize.Data;
                    healthBillOperatorListResponseModel = JsonConvert.DeserializeObject<HealthBillOperatorListResponseModel>(datadeserialize.ToString());
                    if (!string.IsNullOrEmpty(Number))
                    {
                        var dataBillerInfo = healthBillOperatorListResponseModel.billerInfo.Where(x => x.Id == Operator);
                        var billerAdhoc = healthBillOperatorListResponseModel.billerInfo.FirstOrDefault().inputParam.FirstOrDefault().Optional;
                            ...
                            if (deserializeN.Statuscode == "TXN" && deserializeN != null)
                            {
                                ...
                                if (!string.IsNullOrEmpty(Amount) && (!string.IsNullOrEmpty(ccf)) && (!string.IsNullOrEmpty(Payment)))
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        requestPayModel.BillerId = healthBillOperatorListResponseModel.billerInfo.Where(x => x.Id == Operator).FirstOrDefault().Bbps;
                                        ...
                                        requestPayModel.billValidationStatus = healthBillOperatorListResponseModel.billerInfo.Where(x => x.Id == Operator).FirstOrDefault().BillValidation;
                    ...
                    return Json(healthBillOperatorListResponseModel);
                }
                else if (deserialize.Statuscode == "ERR")
                {
                    return Json(deserialize);
