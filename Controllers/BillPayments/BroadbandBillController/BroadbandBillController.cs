...
using Project_Redmil_MVC.Models.RequestModel.BroadbandBillRequestModel;
using Project_Redmil_MVC.Models.RequestModel.ElectricityBillRequestModel;
using Project_Redmil_MVC.Models.ResponseModel;
using Project_Redmil_MVC.Models.ResponseModel.BroadbandBillResponseModel;
using Project_Redmil_MVC.Models.ResponseModel.ElectricityBillResponseModel;
using System.Net.NetworkInformation;

namespace Project_Redmil_MVC.Controllers.BillPayments.BroadbandBillController
{
    public class BroadbandBillController : Controller
    {
        private readonly string Baseurl;
        private readonly IConfiguration _config;
        public GetBroadbandOperatorListResponseModel lstOpResponse;
        public BroadbandBillController(IConfiguration config)
        {
            _config = config;
            Baseurl = HelperMethod.GetBaseURl(_config);
        }
        public IActionResult BroadbandBill()
        {
        ...
        public JsonResult FetchBill(string Number, string Operator, string input1, string input2, string ccf, string Amount, string Payment)
        {
            ...
            try
            {
                ...
                requestModel.State = "";
                requestModel.Category = "Broadband Postpaid";
                ...
                else
                {
                    ...
                    if (deserialize.Statuscode == "TXN" && deserialize != null)
                    {
                        ...
                        lstOpResponse = JsonConvert.DeserializeObject<GetBroadbandOperatorListResponseModel>(datadeserialize.ToString());
                        if (!string.IsNullOrEmpty(Number))
                        {
                            var dataBillerInfo = lstOpResponse.billerInfo.Where(x => x.Id == Operator);
                            var billerAdhoc = lstOpResponse.billerInfo.FirstOrDefault().inputParam.FirstOrDefault().Optional;
                                    ...
                                    if (deserializeN.Statuscode == "TXN" && deserializeN != null)
                                    {
                                        ...
                                        if (!string.IsNullOrEmpty(Amount) && (!string.IsNullOrEmpty(ccf)) && (!string.IsNullOrEmpty(Payment)))
                                        {
                                            ...
                                            try
                                            {
                                                ...
                                                requestPayModel.BillerId = lstOpResponse.billerInfo.Where(x => x.Id == Operator).FirstOrDefault().Bbps;
        ...
        #region GetLandLineOperatorList
        public List<Operatornames> GetOperatorList()
        {
            ...
            try
            {
                ...
                {
                    var datadeserialize = deserialize.Data;
                    var deserializeN = JsonConvert.DeserializeObject<GetBroadbandOperatorListResponseModel>(datadeserialize.ToString());
